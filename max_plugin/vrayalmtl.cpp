...
#include "default_values.h"
	...
	static ParamBlockDesc2 smtl_param_blk(mtl_params, _T("VRayAL parameters"), 0, &SkelMtlCD, P_AUTO_CONSTRUCT + P_AUTO_UI + P_MULTIMAP, 0,
		...
		// params
		mtl_sss_density_scale, _T("sss_density_scale"), TYPE_FLOAT, P_ANIMATABLE, 0,
		...
		mtl_diffuse_strength, _T("diffuse_strength"), TYPE_FLOAT, P_ANIMATABLE, 0,
		...
		mtl_sss_mix, _T("sss_mix"), TYPE_FLOAT, P_ANIMATABLE, 0,
		...
		mtl_sss_mode, _T("sss_mode"), TYPE_INT, 0, 0,
			p_range, 0, 1,
		...
		// SSS
		mtl_sss_color1, _T("sss1_color"), TYPE_RGBA, P_ANIMATABLE, 0,
		...
		mtl_sss_weight1, _T("sss1_weight"), TYPE_FLOAT, P_ANIMATABLE, 0,
		...
		mtl_sss_radius1, _T("sss1_radius"), TYPE_FLOAT, P_ANIMATABLE, 0,
		...
		mtl_sss_color2, _T("sss2_color"), TYPE_RGBA, P_ANIMATABLE, 0,
		...
		mtl_sss_weight2, _T("sss2_weight"), TYPE_FLOAT, P_ANIMATABLE, 0,
		...
		mtl_sss_radius2, _T("sss2_radius"), TYPE_FLOAT, P_ANIMATABLE, 0,
		...
		mtl_sss_color3, _T("sss3_color"), TYPE_RGBA, P_ANIMATABLE, 0,
		...
		mtl_sss_weight3, _T("sss3_weight"), TYPE_FLOAT, P_ANIMATABLE, 0,
		...
		mtl_sss_radius3, _T("sss3_radius"), TYPE_FLOAT, P_ANIMATABLE, 0,
	...
	public:
		...
		SkelMtlParamDlg(SkeletonMaterial *m, HWND hWnd, IMtlParams *i) {
			...
			for (int i=0; i<NUM_PMAPS; i++) {
				...
				TCHAR *title=const_cast<TCHAR*>(pmapInfo[i].title); // Needed for 3ds Max 2012
				pmaps[i]=CreateMParamMap2(i, mtl->pblock, ip, hInstance, hWnd, NULL, NULL, tmp, title, pmapInfo[i].flags, &dlgProc);
		...
		Class_ID ClassID(void) VRAY_OVERRIDE { return VRAYALMTL_CLASSID; }
...
#define MTL_VERSION_CHUNK 0x4001
	...
	void SkeletonMaterial::proc(ILoad *iload) {
		if (version<1) {
			...
			pblock->SetValue(mtl_sss_mode, 0, 1);
	...
	int SkeletonMaterial::getRowBySelectedShortCtrl(HWND hwnd) {
		for (int iTexInfoRow=0; iTexInfoRow<NSUBTEX; ++iTexInfoRow) {
			if (texInfo[iTexInfoRow].texShortID > -1) {
				ParamDef pDef = pblock->GetParamDef(texInfo[iTexInfoRow].texShortID);
				if (pDef.maps.Count() > 0) {
					MapID mapid = pDef.maps[0];
					IParamMap2 *map = pblock->GetMap(mapid);
					if (map) {
						HWND mapWnd=map->GetHWnd();
...
\*===========================================================================*/
	...
	void SkeletonMaterial::Update(TimeValue t, Interval& valid) {
		if (!ivalid.InInterval(t)) {
			...
			pblock->GetValue(mtl_sss_mix, t, sssMix, ivalid);
			pblock->GetValue(mtl_sss_mode, t, sssMode, ivalid);

			pblock->GetValue(mtl_sss_weight1, t, sssWeight1, ivalid);
			pblock->GetValue(mtl_sss_radius1, t, sssRadius1, ivalid);
			pblock->GetValue(mtl_sss_color1, t, sssColor1, ivalid);

			pblock->GetValue(mtl_sss_weight2, t, sssWeight2, ivalid);
			pblock->GetValue(mtl_sss_radius2, t, sssRadius2, ivalid);
			pblock->GetValue(mtl_sss_color2, t, sssColor2, ivalid);

			pblock->GetValue(mtl_sss_weight3, t, sssWeight3, ivalid);
			pblock->GetValue(mtl_sss_radius3, t, sssRadius3, ivalid);
			pblock->GetValue(mtl_sss_color3, t, sssColor3, ivalid);

			pblock->GetValue(mtl_sss_density_scale, t, sssDensityScale, ivalid);
			pblock->GetValue(mtl_diffuse_strength, t, diffuseStrength, ivalid);
